...
if not exist "dist\CdR-Toolkit.exe" (
    echo dist\CdR-Toolkit.exe niet gevonden. Bouw eerst met build_exe.bat.
    pause
    exit /b 1
)

for /f "delims=" %%v in ('python -c "from version import __version__; print(__version__)"') do set "VERSIE=%%v"
for /f "delims=" %%h in ('powershell -NoProfile -Command "(Get-FileHash 'dist\CdR-Toolkit.exe' -Algorithm SHA256).Hash.ToLower()"') do set "HASH=%%h"
